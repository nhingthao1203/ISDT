...
namespace QuanLyHocSinh
{
    public partial class frmPhanLop : Office2007Form
    {
        ...
        private void btnChuyen_Click(object sender, EventArgs e)
        {
            ...
            while (ie.MoveNext())
            {
                ...
                foreach (ListViewItem item in lvLopMoi.Items)
                {
                    ...
                    {
                        MessageBox.Show(
                            $"Student {item.SubItems[1].Text} is currently in class {cmbLopMoi.Text}",
                            "ERROR", 
                ...
                foreach (DataRow row in dataTable.Rows)
                {
                    ...
                    {
                        MessageBox.Show(
                            $"Student {row["HoTen"]} is currently in class {row["TenLop"]}", 
                            "ERROR", 
                            MessageBoxButtons.OK, 
                            MessageBoxIcon.Error
                        );
        ...
        {
            if (MessageBox.Show("Do you want to remove this student from the new class?", "DELETE", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
